        ...
        if rule & 0b010 == 0b010:
            for chessPos in unvisit:
                ...
                if player != chessPlayer:
                    isEaten |= LocalLogic._eatBySymmetries(
                        board, player, chessPos, unvisit, pos
                    )
                else:
        ...
        if rule & 0b001 == 0b001:
            for i in range(5):
                for j in range(5):
                    ...
                    if (
                        chessPlayer != 0
                        and chessPlayer != player
                        and not LocalLogic._getUnmoveChessList(
                            board, chessPlayer, (i, j), visitted
                        )
                    ):
    ...
    def _getUnmoveChessList(board, curChessPlayer, pos, visitted):
        ...
        while i < n and not rt:
            ...
            if chessPlayer == 0:
                ...
                rt = rt or LocalLogic._getUnmoveChessList(
    ...
    def _eatBySymmetries(board, player, pos, unvisit, center=None):
        ...
        for sym in symmetries:
            ...
            if chess1Player == 0 or chess2Player == 0:
                ...
                continue
            if center is not None and (cx != x1 or cy != y1) and (cx != x2 or cy != y2):
                continue
...
def move(board, player):
    ...
    oldBoard[f[1]][f[0]], oldBoard[t[1]][t[0]] = (
        oldBoard[t[1]][t[0]],
        oldBoard[f[1]][f[0]],
    )
