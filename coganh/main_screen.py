...
class PlayerSelect(ScreenFrame):
    def __init__(self, root, player: Player, *args, **kwargs):
        ...
        self.select = ttk.Combobox(
            self, textvariable=self.value, values=options, state="readonly"
        )
        # Pack widgets
    ...
    def pack(self):
        ScreenFrame.pack(self, side="left", padx=40, pady=20, fill="both", expand=True)
...
class ButtonSide(ScreenFrame):
    def __init__(self, *args, **kwargs):
        ...
        self.playButton = ttk.Button(
            self,
            text="Đấu",
            command=lambda: self.root.navigator.push(self.root.play),
            cursor="target",
        )
        ...
        self.playButton.pack(side="bottom", fill="both", expand=True, padx=333, pady=30)
