...
class ChessBoard(tk.Canvas):
        ...
        try:
            ...
            if move[0] in self.movableList and Logic.checkValidMove(move):
                ...
                if not isEaten and Logic.isTrapChess(board, *move):
                    ...
                    self.trapPos = None
                self.movableList = Logic.getMovableChessList(
                    board, self.player.opposite.id, self.trapPos
                )
            else:
    ...
    * - Draw a new circle at position
    ...
    def moveChessman(self, move, **kwargs):
        ...
        self.drawLine(
            move[0], move[1], width=LINE_WIDTH * 3, fill=MOVE_COLOR, tag="temp"
        )
        self.drawCircle(move[0], POINT_SIZE * 3, fill=MOVE_COLOR, width=0, tag="temp")
    ...
    def eatChess(self, pos):
        ...
        self.itemconfig(
            chess.id,
            fill=chess.player.color,
            outline=chess.player.opposite.color,
            width=LINE_WIDTH * 2,
        )
    ...
    def hintClickedChess(self, pos):
        ...
        for mPos in movablePosList:
            self.drawCircle(mPos, POINT_SIZE * 3, fill=HINT_COLOR, width=0, tag="hint")
    ...
    def initializeBoard(self):
        ...
        for i in range(5):
            for j in range(5):
                self.board[i][j] = self.createChessman(initialBoard[i][j], (j, i))
